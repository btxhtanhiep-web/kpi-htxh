...
async function loadProfile(user) {
  ...
  if (accessData.active !== true) {
    const error = new Error(
      "Tài khoản đã bị khóa hoặc ngừng hoạt động. Liên hệ Phòng Tổ chức - Kế toán."
    );
...
const STANDARD_TASK_PILOT = Object.freeze({
  enabled: true,
  mode: "VIEW_ONLY",
  allowedDepartmentIds: Object.freeze(["TCKT"]),
...
function isTchcCoordinationAccount() {
  return (
    normalizeDepartmentId(state.profile?.departmentId) === "TCKT"
