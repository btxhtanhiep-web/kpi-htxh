...
import { TaskNotificationBridge } from "./services/task-notification-bridge.js?v=20260808.V1_10_0";
import {
  openTchcCouncilManager,
  openDepartmentCouncilManager,
  openMyCouncilAdjustments,
  openCouncilReport
} from "./modules/kpi/council-adjustment-ui.js?v=20260808.V1_10_0";

const BUILD = "20260808.V1_10_0";
let observer = null;
let scheduled = false;
...
function scheduleEnhance() {
  ...
  requestAnimationFrame(() => {
    scheduled = false;
    try { enhance(); } catch (error) { console.warn("V1.10.0 enhancement skipped:", error); }
  });
}
...
function hideLegacyStandardTaskFlags() {
  ...
  document.querySelectorAll("small,p,div").forEach(node => {
    const text = exactText(node);
    if (text.includes("Hai cờ “Cốt lõi”") || text.includes('Hai cờ "Cốt lõi"')) {
      node.textContent = "Trường “Đối tượng được nhìn thấy và đăng ký” quyết định quyền hiển thị: Toàn Phòng/Khu – nhân viên và lãnh đạo, hoặc Lãnh đạo quản lý.";
    }
  });
}
...
function bindNotificationMenu() {
  ...
  if (menuButton && menuButton.dataset.bound !== BUILD) {
...
function bootstrap() {
  console.info("Nhiệm vụ & KPI release enhancement", BUILD);
  observer?.disconnect();
