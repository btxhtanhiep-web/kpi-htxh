...
const release = read('nhiem-vu/release-v1.18.5.js');
...
// 1
test('V1.18.5 dùng version/build/cache tập trung và nạp release đúng', () => {
  assert.match(version, /APP_VERSION = "1\.18\.5"/);
  assert.match(version, /BUILD_VERSION = "20260826\.V1_18_5"/);
  assert.match(version, /CACHE_NAME = "nhiem-vu-20260826-v1-18-5"/);
  assert.match(indexHtml, /window\.__APP_HTML_BUILD__ = "20260826\.V1_18_5"/);
  assert.match(indexHtml, /release-v1\.18\.5\.js\?v=20260826\.V1_18_5/);
  assert.match(release, /V1\.18\.5/);
});
...
// 3
test('Session/cache V1.16.1 vẫn được giữ trong V1.18.5', () => {
...
// 51
test('Apps Script tài khoản V3.4.3 có migration V1.18.5 và phân biệt Giám đốc/Phó Giám đốc', () => {
...
// 71
test('Migration V1.18.5 backfill cả task cha và taskMilestones legacy', () => {
  assert.match(accountSync,/migrateProductionDataV1183/);
...
// 94
test('V1.18.5 Firestore Rules không còn helper chết bị Firebase Console từ chối Publish', () => {
  const functionNames = [...rules.matchAll(/\bfunction\s+([A-Za-z_]\w*)\s*\(/g)].map(match => match[1]);
  const unused = functionNames.filter(name => {
...
// 95
test('V1.18.5 tiếp tục loại 5 dead helpers gây lỗi Publish, không nới quyền', () => {
  for (const name of [
    'canWriteExecutiveDepartmentUpdate',
    'isCdtnDeputySecretary',
    'isCdtnCatalogManager',
    'canConfirmDepartmentEvaluations',
    'directorSelfScoreUpdateOnly'
  ]) {
    assert.doesNotMatch(rules, new RegExp(`\\bfunction\\s+${name}\\s*\\(`));
  }
  assert.match(rules, /function canConfirmOwnerForScope\(ownerId, scopeDepartmentId\)/);
  assert.match(rules, /ownerId != request\.auth\.uid/);
  assert.match(rules, /function ownerCompletesMilestoneOnly\(\)/);
  assert.match(rules, /function ownerEventDrivenCompletionValid\(\)/);
});

// 96
test('V1.18.5 giữ nguyên Rules root/deployment sau hotfix', () => {
  assert.equal(rules, deployRules);
  assert.match(rules, /V1\.18\.5: owner-write runtime hotfix/);
});


// 97
test('V1.18.5 ưu tiên ownerCanUpdateTask trước nhánh manager/reviewer/council', () => {
  const block = rules.slice(rules.indexOf('match /tasks/{taskId}'), rules.indexOf('function taskMilestoneVisible'));
  const ownerPos = block.indexOf('ownerCanUpdateTask(taskId)');
  const managePos = block.indexOf('canManageTaskUpdate(resource.data, taskId)');
  assert.ok(ownerPos >= 0 && managePos >= 0 && ownerPos < managePos);
});

// 98
test('V1.18.5 canManageTaskUpdate fail-fast system progress trước canManageTask', () => {
  const block = rules.slice(rules.indexOf('function canManageTaskUpdate'), rules.indexOf('/* ==================== KẾ HOẠCH'));
  assert.ok(block.indexOf('!changesSystemManagedTaskProgress()') < block.indexOf('canManageTask(data)'));
});

// 99
test('V1.18.5 council task request fail-fast trước đọc councilRound', () => {
  const block = rules.slice(rules.indexOf('function validCouncilTaskRequest'), rules.indexOf('function validCouncilEvaluationRequest'));
  assert.ok(block.indexOf('hasField(request.resource.data, "councilAdjustmentRequestId")') < block.indexOf('councilRoundOpen('));
  assert.ok(block.indexOf('diff(resource.data).affectedKeys().hasOnly') < block.indexOf('councilRoundOpen('));
});

// 100
test('V1.18.5 reviewer score fail-fast theo write shape trước ma trận reviewer', () => {
  const block = rules.slice(rules.indexOf('function reviewerTaskScoreUpdateOnly'), rules.indexOf('/* ==================== HỘI ĐỒNG'));
  assert.ok(block.indexOf('diff(resource.data).affectedKeys().hasOnly') < block.indexOf('canConfirmTaskScore(data)'));
});
