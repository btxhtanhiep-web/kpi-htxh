...
const release = read('nhiem-vu/release-v1.18.0.js');
...
const accountSync = read('deployment/APPS_SCRIPT_DONG_BO_TAI_KHOAN_V3_4_0.gs');
...
// 1
test('V1.18.0 dùng version/build/cache tập trung và nạp release đúng', () => {
  assert.match(version, /APP_VERSION = "1\.18\.0"/);
  assert.match(version, /BUILD_VERSION = "20260825\.V1_18_0"/);
  assert.match(version, /CACHE_NAME = "nhiem-vu-20260825-v1-18-0"/);
  assert.match(indexHtml, /window\.__APP_HTML_BUILD__ = "20260825\.V1_18_0"/);
  assert.match(indexHtml, /release-v1\.18\.0\.js\?v=20260825\.V1_18_0/);
  assert.match(release, /V1\.18\.0/);
});
...
// 3
test('Session/cache V1.16.1 vẫn được giữ trong V1.18.0', () => {
...
// 25
test('Ủy quyền BGD chỉ cho Phó Giám đốc và quyền CONFIRM_EVALUATIONS', () => {
  assert.match(rules,/data\.departmentId == "BGD" && isDirectorDeputyProfile/);
  assert.match(rules,/data\.permissions\.toSet\(\)\.hasOnly\(\["CONFIRM_EVALUATIONS"\]\)/);
});
...
// 51
test('Apps Script tài khoản V3.4.0 có migration V1.18.0 và phân biệt Giám đốc/Phó Giám đốc', () => {
  assert.match(accountSync,/VERSION: '3\.4\.0'/);
  assert.match(accountSync,/migrateProductionDataV118/);
