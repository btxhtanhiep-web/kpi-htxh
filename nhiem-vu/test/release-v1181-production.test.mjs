...
const release = read('nhiem-vu/release-v1.18.1.js');
...
const accountSync = read('deployment/APPS_SCRIPT_DONG_BO_TAI_KHOAN_V3_4_1.gs');
...
// 1
test('V1.18.1 dùng version/build/cache tập trung và nạp release đúng', () => {
  assert.match(version, /APP_VERSION = "1\.18\.1"/);
  assert.match(version, /BUILD_VERSION = "20260826\.V1_18_1"/);
  assert.match(version, /CACHE_NAME = "nhiem-vu-20260826-v1-18-1"/);
  assert.match(indexHtml, /window\.__APP_HTML_BUILD__ = "20260826\.V1_18_1"/);
  assert.match(indexHtml, /release-v1\.18\.1\.js\?v=20260826\.V1_18_1/);
  assert.match(release, /V1\.18\.1/);
});
...
// 3
test('Session/cache V1.16.1 vẫn được giữ trong V1.18.1', () => {
...
// 49
test('Phần C chỉ lấy nhiệm vụ bonus official/approved và liệt kê từng công việc', () => {
  assert.match(reportBlock,/score\.official && item\.score\.bonusAwarded && item\.score\.bonusScore > 0/);
  assert.match(reportBlock,/m01-bonus-item/);
  assert.match(reportBlock,/TỔNG \(C\)/);
  assert.match(reportBlock,/Tối đa 07 điểm/);
});
...
// 51
test('Apps Script tài khoản V3.4.1 có migration V1.18.1 và phân biệt Giám đốc/Phó Giám đốc', () => {
  assert.match(accountSync,/VERSION: '3\.4\.1'/);
  assert.match(accountSync,/migrateProductionDataV1181/);
...
// 66
test('Mẫu 01A/01B dùng lưới 6 cột hợp lý, STT 5% và nội dung 43%', () => {
  const css=read('nhiem-vu/v3.css');
  assert.match(reportBlock,/<colgroup>[\s\S]*m01-col-stt[\s\S]*m01-col-note[\s\S]*<\/colgroup>/);
  assert.match(reportBlock,/m01-task-row[\s\S]*<td colspan="2">/);
  assert.match(css,/\.m01-col-stt\{width:5%\}/);
  assert.match(css,/\.m01-col-content\{width:43%\}/);
  assert.match(css,/\.m01-col-max\{width:10%\}/);
  assert.match(css,/\.m01-col-self\{width:12%\}/);
  assert.match(css,/\.m01-col-authority\{width:18%\}/);
  assert.match(css,/\.m01-col-note\{width:12%\}/);
});
