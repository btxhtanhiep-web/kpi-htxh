...
const release = read('nhiem-vu/release-v1.18.2.js');
...
const accountSync = read('deployment/APPS_SCRIPT_DONG_BO_TAI_KHOAN_V3_4_2.gs');
...
// 1
test('V1.18.2 dùng version/build/cache tập trung và nạp release đúng', () => {
  assert.match(version, /APP_VERSION = "1\.18\.2"/);
  assert.match(version, /BUILD_VERSION = "20260826\.V1_18_2"/);
  assert.match(version, /CACHE_NAME = "nhiem-vu-20260826-v1-18-2"/);
  assert.match(indexHtml, /window\.__APP_HTML_BUILD__ = "20260826\.V1_18_2"/);
  assert.match(indexHtml, /release-v1\.18\.2\.js\?v=20260826\.V1_18_2/);
  assert.match(release, /V1\.18\.2/);
});
...
// 3
test('Session/cache V1.16.1 vẫn được giữ trong V1.18.2', () => {
...
// 51
test('Apps Script tài khoản V3.4.2 có migration V1.18.2 và phân biệt Giám đốc/Phó Giám đốc', () => {
  assert.match(accountSync,/VERSION: '3\.4\.2'/);
  assert.match(accountSync,/migrateProductionDataV1182/);
...
// 70
test('EVENT_DRIVEN close có timeout và kiểm tra lại server, không chờ vô hạn', () => {
  assert.match(taskWrite,/EVENT_CLOSE_COMMIT_TIMEOUT_MS = 15000/);
  assert.match(taskWrite,/commitEventCloseWithRecovery/);
  assert.match(taskWrite,/getDocFromServer/);
  assert.match(taskWrite,/write-confirmation-timeout/);
});

// 71
test('Migration V1.18.2 backfill schema milestone legacy từ taskMilestones', () => {
  assert.match(accountSync,/migrateProductionDataV1182/);
  ...
  assert.match(accountSync,/listFirestoreCollectionDocuments_\(auth, 'taskMilestones'\)/);
});
...
test('Rules production giữ milestone strict; migration sửa legacy thay vì nới owner sửa cấu hình', () => {
  ...
  assert.match(rules,/V1\.18\.2: dùng migration để backfill milestone legacy/);
});
