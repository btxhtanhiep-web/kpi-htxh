...
const release = read('nhiem-vu/release-v1.18.4.js');
...
// 1
test('V1.18.4 dùng version/build/cache tập trung và nạp release đúng', () => {
  assert.match(version, /APP_VERSION = "1\.18\.4"/);
  assert.match(version, /BUILD_VERSION = "20260826\.V1_18_4"/);
  assert.match(version, /CACHE_NAME = "nhiem-vu-20260826-v1-18-4"/);
  assert.match(indexHtml, /window\.__APP_HTML_BUILD__ = "20260826\.V1_18_4"/);
  assert.match(indexHtml, /release-v1\.18\.4\.js\?v=20260826\.V1_18_4/);
  assert.match(release, /V1\.18\.4/);
});
...
// 3
test('Session/cache V1.16.1 vẫn được giữ trong V1.18.4', () => {
...
// 51
test('Apps Script tài khoản V3.4.3 có migration V1.18.4 và phân biệt Giám đốc/Phó Giám đốc', () => {
...
// 71
test('Migration V1.18.4 backfill cả task cha và taskMilestones legacy', () => {
  assert.match(accountSync,/migrateProductionDataV1183/);
...
// 94
test('V1.18.4 Firestore Rules không còn helper chết bị Firebase Console từ chối Publish', () => {
...
// 95
test('V1.18.4 loại đúng 5 dead helpers gây lỗi Publish, không nới quyền', () => {
  for (const name of [
...
// 96
test('V1.18.4 giữ nguyên Rules root/deployment sau hotfix', () => {
  assert.equal(rules, deployRules);
  assert.match(rules, /V1\.18\.4: Rules publish hotfix/);
});
