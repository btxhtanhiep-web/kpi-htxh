...
function bindLogout() {
  ...
  const logout = async () => {
    ...
    try {
      /* Notification OFF ở tenant 60 người: không phát sinh read/write subscription. */
      const snapshot = NOTIFICATIONS_ENABLED
        ? await window.TaskPush?.getSubscriptionSnapshot?.().catch(() => null)
        : null;
