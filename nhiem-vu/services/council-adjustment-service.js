...
function isTchcHead(user = UserContext.getUser()) {
  return Permissions.isAdmin(user) || (
    Permissions.isDepartmentHead(user) && upper(user?.departmentId) === "TCKT"
  );
}
...
async function ensureOpenRound(periodId, departmentId = "") {
  ...
  if (departmentId) {
    ...
    if (!departmentSnapshot.exists() || departmentSnapshot.data()?.enabled !== true) {
      throw new Error("Phòng/Khu này chưa được TCKT mở quyền điều chỉnh sau Hội đồng.");
    }
  }
  return round;
}
...
export const CouncilAdjustmentService = Object.freeze({
  ...
  async openRound(period, departmentIds = []) {
    ...
    if (!isTchcHead(actor)) throw new Error("Chỉ Trưởng Phòng Tổ chức - Kế toán được mở đợt điều chỉnh sau Hội đồng.");
  ...
  async closeRound(periodId) {
    const actor = UserContext.requireUser();
    if (!isTchcHead(actor)) throw new Error("Chỉ Trưởng Phòng Tổ chức - Kế toán được khóa đợt điều chỉnh sau Hội đồng.");
