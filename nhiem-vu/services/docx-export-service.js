...
function topHeaderXml(top) {
  ...
  if (/01\s*-?\s*B/i.test(formText)) {
    const idx = agencyLines.findIndex(line => /^TRUNG TÂM\s+HỖ TRỢ XÃ HỘI$/i.test(line));
    if (idx >= 0) {
      agencyLines = [
...
export function buildDocxBlobFromElement(root, { title = 'Báo cáo KPI cá nhân', creator = 'Trung tâm Hỗ trợ xã hội' } = {}) {
