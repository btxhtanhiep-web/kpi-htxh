...
export const UserNotificationService = Object.freeze({
  subscribeCurrentUser(onData, onError) {
    if (!NOTIFICATIONS_ENABLED) { onData?.([]); return () => {}; }
  ...
  notifyTaskAction(action, taskId, eventData = {}, eventId = "") {
    if (!NOTIFICATIONS_ENABLED) return Promise.resolve(false);
    return notifyManagersFromTaskAction(action, taskId, eventData, eventId);
  },

  notifyRegistrationDecision(registration, reviewer, status, options = {}) {
    if (!NOTIFICATIONS_ENABLED) return Promise.resolve(false);
    return notifyRegistrationDecision(registration, reviewer, status, options);
  }
});
