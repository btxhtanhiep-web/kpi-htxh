...
import { ExecutiveNotificationService } from "./executive-notification-service.js?v=20260916.V1_24_7_HTXH_1";
...
function canReadOwnDirectiveDepartment(user = UserContext.getUser()) {
  return Permissions.isDepartmentHead(user)
    ...
    || (Permissions.isTchcCoordinator(user) && upper(user?.departmentId) === "TCKT");
}
...
function isDepartmentOperator(user, departmentId) {
  ...
  return target === "TCKT" && upper(user?.role) === "TCHC_COORDINATOR";
}
...
export const ExecutiveDirectiveService = Object.freeze({
  ...
  async createOralDirective(input = {}) {
    ...
    if (tchcRelay && requestedDepartmentId) {
      /* TCKT là đầu mối relay: kể cả giao cho chính TCKT vẫn giữ provenance và đơn vị nhận phải tiếp nhận theo workflow. */
  ...
  async acceptDirective(directive = {}, departmentId = "") {
    ...
    if (!canUserAccept(directive, targetDepartmentId, user)) {
      throw new Error("Chỉ Trưởng/Phó Phòng/Khu (hoặc đầu mối TCKT được cấp quyền) mới được xác nhận tiếp nhận.");
    }
