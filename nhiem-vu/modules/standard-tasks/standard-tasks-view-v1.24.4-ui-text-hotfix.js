...
import { StandardTaskReadService } from "../../services/standard-task-read-service.js?v=20260916.V1_24_7_HTXH_1";
import { PeriodReadService } from "../../services/period-read-service.js?v=20260916.V1_24_7_HTXH_1";
import { StandardTaskWriteService } from "../../services/standard-task-write-service.js?v=20260916.V1_24_7_HTXH_1";
import { TaskRegistrationService } from "../../services/task-registration-service.js?v=20260916.V1_24_7_HTXH_1";
import { deriveDeadlinePlan, deadlineRuleDescription, requiresManualDeadline, isEventDrivenFrequency, canonicalFrequency, STANDARD_FREQUENCIES, WEEKDAY_OPTIONS } from "../../core/deadline-engine.js?v=20260916.V1_24_7_HTXH_1";
...
function preparePersonalRegistrationDetails(items = [], period = {}) {
  return new Promise(resolve => {
    ...
    const addRow = (section, values = {}, isFirst = false) => {
      ...
      row.innerHTML = `<div class="standard-personal-row-head"><strong>${isFirst ? "Đầu việc cá nhân" : "Đầu việc bổ sung"}</strong>${isFirst ? "" : '<button type="button" class="icon-button danger" data-remove-personal-row aria-label="Xóa công việc">×</button>'}</div>
...
async function openTaskEditor(item) {
  ...
  const root = openStandardModal(
    ...
    `<div class="standard-task-editor-intro">
      ...
      <span>Mã được cấp theo từng Phòng/Khu và từng tính chất: thường xuyên dạng TCKT01; đột xuất dạng TCKT-DX01. Mã mới luôn tăng theo số lớn nhất hiện có; không quay lại 01 hoặc lấp khoảng trống.</span>
    </div>
