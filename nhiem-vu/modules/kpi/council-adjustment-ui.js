...
function isTchcHead(user = UserContext.getUser()) {
  return Permissions.isAdmin(user) || (Permissions.isDepartmentHead(user) && upper(user?.departmentId) === "TCKT");
}
...
export async function openTchcCouncilManager() {
  const user = UserContext.requireUser();
  if (!isTchcHead(user)) return ModalService.alert("Chỉ Trưởng Phòng Tổ chức - Kế toán được quản lý đợt điều chỉnh sau Hội đồng.");
