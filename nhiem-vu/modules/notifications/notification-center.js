...
import { UserNotificationService } from "../../services/user-notification-service.js?v=20260911.V1_23_1";
...
function ensureModal() {
  ...
  modal.innerHTML = `
    <section class="app-modal-card notification-center-card">
      <header class="app-modal-header">
        <div>
          <span class="modal-kicker">Tài khoản hiện tại</span>
          <h2>🔔 Thông báo</h2>
          <p>Theo dõi các phê duyệt và cập nhật nhiệm vụ liên quan đến bạn.</p>
        </div>

        <button
          id="btnCloseNotificationCenter"
      ...
      <div class="notification-center-toolbar">
        <button
          id="btnMarkAllNotificationsRead"
        ...
        <button
          id="btnOpenPushSettingsFromNotifications"
          class="secondary-button"
          type="button"
        >
          ⚙ Cài đặt Push
  ...
  modal
    .querySelector("#btnCloseNotificationCenter")
  ...
  modal
    .querySelector("#btnOpenPushSettingsFromNotifications")
    ?.addEventListener("click", () => {
      close();
      window.dispatchEvent(
        new CustomEvent("app:open-push-settings")
      );
    });

  modal
    .querySelector("#btnMarkAllNotificationsRead")
    ?.addEventListener("click", async event => {
      event.currentTarget.disabled = true;

      try {
        await UserNotificationService.markAllRead(items);
      } catch (error) {
        ToastService.error(
...
function renderList() {
  ...
  if (!items.length) {
    list.innerHTML = `
      <div class="notification-empty">
        ...
        <p>
          Các phê duyệt và cập nhật liên quan sẽ xuất hiện tại đây.
        </p>
      </div>
    `;
    return;
  ...
  list.innerHTML = items.map(item => `
    <button
      class="notification-item ${item.read === true ? "is-read" : "is-unread"}"
      type="button"
    ...
    >
      ...
      <span class="notification-item-body">
        ...
        <small>
          ...
          ${item.taskCode ? ` · ${escapeHtml(item.taskCode)}` : ""}
        </small>
      </span>
    </button>
  `).join("");
...
function close() {
  document
    .getElementById("notificationCenterModal")
...
function bindOpenButtons() {
  ...
  buttons.forEach(button => {
    ...
    const textNode = button.querySelector(
      "span:not(.notification-unread-badge)"
    );
    ...
    if (
      textNode &&
      button.id === "btnPushSettings"
    ) {
      textNode.textContent = "Thông báo";
    }
...
export const NotificationCenter = Object.freeze({
  start(user) {
    ...
    unsubscribe =
      UserNotificationService.subscribeCurrentUser(
        data => {
          items = data;
          updateBadges();
        ...
        error =>
          console.warn(
            "Không theo dõi được Notification Center:",
            error
          )
      );
