 ...
 */
import { UserContext } from "./user-context.js?v=20260916.V1_24_7_HTXH_1";
...
export const Permissions = Object.freeze({
  ...
  canManageEvaluationPeriods() {
    const user = UserContext.getUser();
    return this.isAdmin(user) || (this.isDepartmentHead(user) && upper(user?.departmentId) === "TCKT");
  },
  ...
  canManageExecutiveDirectives(user = UserContext.getUser()) {
    ...
    if (this.isDirector(user)) return true;
    if (upper(user?.departmentId) !== "TCKT") return false;
  ...
  canRecordOralExecutiveDirective(user = UserContext.getUser()) {
    ...
    /* Phòng/Khu thông thường: chỉ Trưởng/Phụ trách (HEAD) ghi nhận BGĐ cho chính đơn vị. TCKT Phó đi qua capability relay riêng. */
  ...
  canRelayOralExecutiveDirective(user = UserContext.getUser()) {
    /*
     * V1.22.0: TCKT là đầu mối ghi nhận/chuyển tải chỉ đạo miệng BGĐ xuống Phòng/Khu.
     * System privilege ADMIN không tự tạo capability này; phải có business position/capability TCKT phù hợp.
     */
    return activeUser(user)
      && upper(user?.departmentId) === "TCKT"
      && (
